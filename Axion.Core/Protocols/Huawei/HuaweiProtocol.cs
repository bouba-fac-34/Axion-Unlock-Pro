...
namespace Axion.Core.Protocols.Huawei
{
    public class HuaweiProtocol
    {
        private readonly string _adb;
        private readonly Action<string> _log;

        public HuaweiProtocol(string adb, Action<string> log) { _adb = adb; _log = log; }
        ...
        public async Task<OperationResult> RemoveFrpAsync(DeviceInfo d)
        {
            ...
            await Adb(d.Serial, "shell pm clear com.huawei.hwid");
            ...
            sw.Stop();
            return Ok("Huawei FRP cleared", log.ToString(), sw.Elapsed);
        }
        ...
        public async Task<OperationResult> DisableOtaAsync(DeviceInfo d)
        {
            ...
            await Adb(d.Serial, "shell pm disable-user --user 0 com.huawei.android.hwouc");
            L("OTA disabled");
            sw.Stop();
            return Ok("Huawei OTA disabled", log.ToString(), sw.Elapsed);
        }
