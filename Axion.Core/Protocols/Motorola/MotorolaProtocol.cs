...
namespace Axion.Core.Protocols.Motorola
{
    public class MotorolaProtocol
    {
        private readonly string _adb;
        private readonly Action<string> _log;

        public MotorolaProtocol(string adb, Action<string> log) { _adb = adb; _log = log; }
        ...
        public async Task<OperationResult> RemoveFrpAsync(DeviceInfo d)
        {
            ...
            await Adb(d.Serial, "shell pm clear com.motorola.android.providers.settings");
            ...
            sw.Stop();
            return Ok("Motorola FRP cleared", log.ToString(), sw.Elapsed);
        }
        ...
        public async Task<OperationResult> DisableOtaAsync(DeviceInfo d)
        {
            ...
            L("OTA disabled");
            sw.Stop();
            return Ok("Motorola OTA disabled", log.ToString(), sw.Elapsed);
        }
